...
The functions ~dap-java-run-test-class~ and ~dap-java-run-test-method~ expect
the variable ~dap-java-test-runner~ to point to a JUnit JAR. You can download
one [[https://repo1.maven.org/maven2/org/junit/platform/junit-platform-console-standalone/][from Maven Central]].
